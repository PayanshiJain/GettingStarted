...
public class Main {
  ...
  public static int size(Node node) {
    ...
    //faith-> bring size of left side and right side
  ...
  public static int max(Node node) {
    ...
    // int maximum=Math.max(leftside,rightside);
    // int result=Math.max(maximum,node);
    // return result;
    //OR
    int result=Math.max(node.data, Math.max(leftside,rightside));
    return result;
  }
...
public class Main {
  ...
  public static void iterativePrePostInTraversal(Node node) {
    ...
    while(st.size()>0){
      Pair tos = st.peek();   //tos-> top of stack 
...
//FIND AND NODETOROOT PATH IN BINARY TREE
import java.io.*;
import java.util.*;
...
public class Main {
  ...
  public static ArrayList<Integer> nodeToRootPath(Node node, int data){
    ...
    //if not = then search in left part
    ...
    //if kahi bhi node nhi mila then return blank arr list
    return new ArrayList<>();
  }
...
public class Main {
  ...
  public static void printKNodesFar(Node node, int data, int k) {
    ...
    for(int i=0;i<al.size();i++){
      Node cr=al.get(i); //cr-> current node
      Node blocker= i==0 ? null:al.get(i-1); //ternary operator, if i=0, 1st condition will be executed otherwise 2nd
      //check notes for details about ternary operator
      printKLevelsDown(cr,k-i,blocker);
      //bcz 1st element of al k liye k levels down, 2nd element uske upar hoga i.e.37k toh usse k-1 levels down
      //then 3rd element of al se k-2 levels down therefore it is k-i 
    }
    ...
    // }
    // for(int i=0;i<=k;i++){
    //   printKLevelDown(node.left,k-i);
    //   printKLevelDown(node.right,k-i);
    // }
  }
...
public class Main {
  ...
  public static void createLeftCloneTree(Node node){
    ...
    //the above line means Node(data,left,right)
...
public class Main {
  ...
  static int dia=0;  //global variable
  public static int diameter1(Node node) {
    // write your code here
    if(node==null){
      return -1; //bcz we r talking in terms of edge
    }
    int lh= diameter1(node.left); //left height
    int rh= diameter1(node.right);  //right height
...
public class Main {
  ...
  public static int tilt(Node node){
    ...
    int ls=tilt(node.left);  //get sum of ls
    int rs=tilt(node.right);  //get sum of rs
    tilt+=Math.abs(ls-rs);
...
public class Main {
  ...
  static boolean balance=true;  //declaration of global var, consider balance as true by default
  public static int isBalance(Node node){
    if(node==null){
      return -1;
    }
    int lsh=isBalance(node.left); //left subtree height
    int rsh=isBalance(node.right); //height of right subtree

    if(Math.abs(lsh-rsh)>1){
      balance=false;;
    }
    return Math.max(rsh,lsh)+1;  //return height 
